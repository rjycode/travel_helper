...
class DialogueEngine:
    ...
    async def handle_message(self,
        ...
        """
        职责：（调用LLM 做路由分析、校验分析后的结果、进入到对应轨道内部处理、推进流程..）
        ...
        # 3.2 对象消息类型
            ...
            bot_messages = await self._handle_object_message(user_message.object, dialogue_state, self.task_handler.flow_list)
    ...
    async def _handle_text_message(self,dialogue_state: DialogueState) -> list[BotMessage]:
        ...
        #4 校验成功(到底是哪一条轨道，进入到轨道内部去执行对应的轨道内逻辑【xxxHandler】)
    ...
    async def _handle_object_message(self,
        ...
        """
        职责：处理对象消息类型,本质构建SetSlotsCommand对象
        :param object:
        :param dialogue_state:
        :param flow_list:
        :return:
